...
$pdf->Cell(245,10, ('Reporte completo de Libros: '), 0, 1, 'L');
...
$pdf->MultiCell(w:0, h:10, txt: 'En la siguiente tabla se muestra el reporte completo de todos los libros existentes en la '
.'Biblioteca del Centro de Bachillerato Forestal 1 (CBTF1) hasta la fecha: '.utf8_decode($diassemana[date('w')]." ".date('d')." de ".$meses[date('n')-1]. " del ".date('Y'))
.'.');
...
$pdf->Cell(35,10, utf8_decode(('Autor')), 1, 0, 'C',1);
$pdf->Cell(15,10, utf8_decode(('Ubic.')), 1, 0, 'C',1);
$pdf->Cell(20,10, utf8_decode(('Paginas')), 1, 0, 'C',1);
$pdf->Cell(10,10, utf8_decode(('Edic')), 1, 0, 'C',1);
$pdf->Cell(25,10, utf8_decode(('Editorial')), 1, 0, 'C',1);
$pdf->Cell(35,10, utf8_decode(('Categoria')), 1, 0, 'C',1);
$pdf->Cell(20,10, utf8_decode(('F. Adq.')), 1, 0, 'C',1);
$pdf->Cell(20,10, utf8_decode(('Folio')), 1, 1, 'C',1);
...
while ($row = mysqli_fetch_assoc($buscador)) { 
    ...
    $pdf->Cell(35,10, utf8_decode($row['autor1']), 1, 0, 'C',1);
    $pdf->Cell(15,10, utf8_decode($row['ubicacion']).' - '.($row['celda']), 1, 0, 'C',1);
    $pdf->Cell(20,10, utf8_decode($row['paginas']), 1, 0, 'C',1);
    $pdf->Cell(10,10, utf8_decode($row['edicion']), 1, 0, 'C',1);
    $pdf->Cell(25,10, utf8_decode($row['editorial']), 1, 0, 'C',1);
    $pdf->Cell(35,10, utf8_decode($row['cat1']), 1, 0, 'C',1);
    $pdf->Cell(20,10, utf8_decode((str_replace('-', '/', date('d-m-Y', strtotime($row['fechaad']))))), 1, 0, 'C',1);
    $pdf->Cell(20,10, utf8_decode($row['folio']), 1, 1, 'C',1);

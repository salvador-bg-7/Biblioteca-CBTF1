...
$fed = $mysqli->real_escape_string($_POST['fed']);
...
$pdf->Cell(80,10, (''), 0, 1, 'R');
$pdf->Cell(160,10, utf8_decode('Reporte de préstamo del Libro: '), 0, 1, 'L');
...
$pdf->MultiCell(w:0, h:10,  txt: utf8_decode('El Libro '.$tit.' se entrego en préstamo a '.$nos.' con matricula '.$mas.
', por parte de la Biblioteca del Centro de Bachillerato Tecnológico Forestal 1 (CBTF1) en la fecha de '
.(str_replace('-', '/', date('d-m-Y', strtotime($fes)))).' y fue devuelto en la fecha de '.(str_replace('-', '/', date('d-m-Y', strtotime($fed)))).' en las condiciones que a continuacion se describe: '.$mot.'.'), border:0, align:'J');

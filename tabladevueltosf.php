...
$pdf->Cell(245,10, ('Reporte de libros devueltos: '), 0, 1, 'L');
...
$pdf->MultiCell(w:0, h:10, txt: utf8_decode
('En la siguiente tabla se muestra el reporte de los libros que han sido devueltos por alumnos, maestros y personal administrativo a la '
.'Biblioteca del Centro de Bachillerato Tecnológico Forestal 1 (CBTF1) desde la fecha: '.utf8_decode(str_replace('-', '/', date('d-m-Y', strtotime($fecha1)))).
' hasta la fecha: '.utf8_decode(str_replace('-', '/', date('d-m-Y', strtotime($fecha2)))).'.'));
...
$pdf->Cell(50,10, utf8_decode(('Titulo')), 1, 0, 'C',1);
$pdf->Cell(15,10, utf8_decode(('Folio')), 1, 0, 'C',1);
$pdf->Cell(50,10, utf8_decode(('Nombre Solicitante')), 1, 0, 'C',1);
$pdf->Cell(40,10, utf8_decode(('Matr. Solicitante')), 1, 0, 'C',1);
$pdf->Cell(40,10, utf8_decode(('Fecha de Solicitud')), 1, 0, 'C',1);
$pdf->Cell(40,10, utf8_decode(('Fecha de Entrega')), 1, 1, 'C',1);
...
while ($row = mysqli_fetch_assoc($buscador)) { 
    ...
    $pdf->Cell(50,10, utf8_decode($row['titulo']), 1, 0, 'C',1);
    $pdf->Cell(15,10, utf8_decode($row['folio']), 1, 0, 'C',1);
    $pdf->Cell(50,10, utf8_decode($row['nombresoli']), 1, 0, 'C',1);
    $pdf->Cell(40,10, utf8_decode($row['matriculasoli']), 1, 0, 'C',1);
    $pdf->Cell(40,10, utf8_decode(str_replace('-', '/', date('d-m-Y', strtotime($row['fechaPedido'])))), 1, 0, 'C',1);
    $pdf->Cell(40,10, utf8_decode(str_replace('-', '/', date('d-m-Y', strtotime($row['fecha'])))), 1, 1, 'C',1);
